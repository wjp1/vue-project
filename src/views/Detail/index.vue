    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :sendImageList="skuImageList" />
          <!-- 小图列表 -->
          <ImgList :sendImageList="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="items,index in spuSaleAttrList" :key="index">
                <dt class="title">{{items.saleAttrName}}</dt>
                <!-- 商品的属性选项按钮 -->
                <dd changepirce="0" :class="{'active':item.isChecked==1}" 
                  v-for="item in items.spuSaleAttrValueList" :key="item.id"
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins" @click="skuNum <= 1? skuNum = 1 : skuNum=skuNum-1">-</a>
              </div>
              <div class="add">
                <a @click="addOrUpdateShopCar">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
    ...
    methods:{
      //产品售卖属性值高亮
      changeActive(saleAttrValue,saleAttrList){
      ...
      async addOrUpdateShopCar(){
        //发请求给服务器
        try {
          await this.$store.dispatch('postShopCartInfo',{skuId:this.$route.params.goodsid,skuNum:this.skuNum})
          //跳转路由
          //简单的数据通过query或params传
          //复杂的数据通过会话存储，(会话结束，数据消失)
